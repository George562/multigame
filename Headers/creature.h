...
public:
    ...
    virtual void move(Location* location) {
        ...
        if (location->doorPos != sf::Vector2i(0, 0)) {
            ...
            if (door_collision.x == -1) {
                if (this->inventory.items.count(ItemID::keyCard) > 0 && this->inventory.items[ItemID::keyCard]->amount > 0) {
                    DeleteWall(location->walls, location->wallsRect, location->doorPos.y, location->doorPos.x);
                    this->inventory.items[ItemID::keyCard]->amount = 0;
                } else Velocity.x = 0;
            } else if (door_collision.y == -1) {
                if (this->inventory.items.count(ItemID::keyCard) > 0 && this->inventory.items[ItemID::keyCard]->amount > 0) {
                    DeleteWall(location->walls, location->wallsRect, location->doorPos.y, location->doorPos.x);
                    this->inventory.items[ItemID::keyCard]->amount = 0;
                } else Velocity.y = 0;
