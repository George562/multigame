...
public:
    ...
    void Propagation() {
        ...
        descendant1 = new Fire(PosX + dist * std::sin(angle), PosY + dist * std::cos(angle), Width, Height, sf::seconds(rand() % 4 + 1));
        angle = (rand() % 360) * M_PI / 180;
        descendant2 = new Fire(PosX + dist * std::sin(angle), PosY + dist * std::cos(angle), Width, Height, sf::seconds(rand() % 4 + 1));
    }
